...
public class PacInfo implements Constants{
	...
	*/
	...
	public synchronized Node getPowerPillIndex(GameStateInterface gs)
	{
		return getPowerPillIndex(new SimGameState(gs));
	}
	...
	public synchronized  Node getPowerPillIndex(SimGameState gs){
		Node index = null;
		for (Node n : maze.getPowers()){
			if (gs.getPowers().get(n.powerIndex)){
				 index = n;
